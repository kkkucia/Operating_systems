...
int create_unix_connection(char *path) {
...
void create_connection(char **argv) {
    if (strcmp(argv[2], "unix") == 0) {
        sock = create_unix_connection(argv[3]);
...
void handle_input() {
    ...
    while (token != NULL) {
        switch (idx) {
            ...
            case 2:
                ...
                other_nickname[sizeof(other_nickname) - 1] = '\0';
                break;
            case 3:
                broke = 1;
                break;
    ...
    message msg;
    msg.type = type;
    strncpy(msg.other_nickname, other_nickname, sizeof(msg.other_nickname));
