...
Blocks *block_init(char *result, Blocks *(*create_blocks)(int)) {
    result = strtok(NULL, " ");
...
void block_count(Blocks *blocks, char *result, void (*count_words_and_lines)(Blocks *, char *)) {
    result = strtok(NULL, " ");
...
void block_show(Blocks *blocks, char *result, char *(*get_block)(Blocks *, int)) {
    result = strtok(NULL, " ");
...
void block_delete(Blocks *blocks, char *result, void (*remove_block)(Blocks *, int)) {
    result = strtok(NULL, " ");
...
int main(int argc, char **argv) {
    ...
    Blocks *(*create_blocks)(int) = dlsym(handle, "create_blocks");
    void (*count_words_and_lines)(Blocks *, char *) = dlsym(handle, "count_words_and_lines");
    char *(*get_block)(Blocks *, int) = dlsym(handle, "get_block");
    void (*remove_block)(Blocks *, int) = dlsym(handle, "remove_block");
    void (*free_blocks)(Blocks *) = dlsym(handle, "free_blocks");

    if (!handle) {
        fprintf(stderr, "ERORR when loading liblibrary.so\n");
        return 0;
    }
    ...
    while (fgets(result, MAX_COMMAND_SIZE, stdin)) {
        ...
        if (strcmp(argument, "help") == 0) {
            ...
            start_timer();
            blocks = block_init(result, create_blocks);
            end_timer();
        } else if (strcmp(argument, "count") == 0) {
            start_timer();
            block_count(blocks, result, count_words_and_lines);
            end_timer();
        } else if (strcmp(argument, "show") == 0) {
            start_timer();
            block_show(blocks, result, get_block);
            end_timer();
        } else if (strcmp(argument, "delete") == 0) {
            start_timer();
            block_delete(blocks, result, remove_block);
            end_timer();
